/**
 * Completa artículos teenagers incompletos (ES + EN) al 100%.
 * Uso: node scripts/complete-teenagers-content.js
 */
const fs = require('fs');
const path = require('path');
const { fetch, normalizeTitle, buildSlugMaps } = require('./content-utils');
const { CATEGORY_URLS, TEENAGERS_TOPICS } = require('./category-urls');
...
const CATEGORY = 'teenagers';
const KIDSHEALTH_PATH = 'teens';
...
function buildUrlCandidates(article, lang) {
  ...
  for (const slug of slugCandidates(article, lang)) {
    urls.add(`https://kidshealth.org/${lang}/${KIDSHEALTH_PATH}/${slug}.html`);
    urls.add(`https://kidshealth.org/${lang}/${KIDSHEALTH_PATH}/center/${slug}.html`);
...
async function main() {
  ...
  for (const topic of TEENAGERS_TOPICS) {
  ...
  console.log(`\n🔧 Completando ${incomplete.length} artículos teenagers incompletos...\n`);
